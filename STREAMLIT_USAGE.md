...
- 🎨 **直观的用户界面**：简洁美观的全中文 Web 界面，易于使用
- 📝 **灵活的提示词输入**：支持自定义提示词和预设示例
- ⚙️ **完整的参数控制**：可调整图像尺寸、推理步数、种子等参数
- 📏 **智能尺寸调整**：自动将输入的宽高调整为16的倍数，避免生成错误
- 🖼️ **实时图像预览**：生成后立即显示图像
- ⬇️ **一键下载**：支持直接下载生成的图像
...
- **模型路径**：模型文件路径（默认：`ckpts/Z-Image-Turbo`）
- **计算设备**：选择计算设备
  - `cuda`：使用 GPU（推荐，速度快）
  - `cpu`：使用 CPU（较慢）
- **数据类型**：模型精度
...
- **编译模型**：启用模型编译以加速推理（首次运行会较慢）

### 2. 生成参数（侧边栏）

调整图像生成参数：

- **高度 / 宽度**：输出图像的尺寸（512-2048 像素）
  - 推荐：1024x1024
  - **智能调整**：系统会自动将输入值调整为16的倍数，例如输入1920x1080会自动调整为1920x1072或1088
  - 如果输入值被调整，界面会显示提示信息告知实际使用的尺寸
  - 注意：更大的尺寸需要更多显存和时间
- **推理步数**：推理步数（1-50）
  - 推荐：8（Turbo 模型优化的步数）
  - 更多步数可能提高质量，但会增加生成时间
- **引导系数**：分类器自由引导比例（0.0-10.0）
  - 推荐：0.0（Turbo 模型优化设置）
- **随机种子**：随机种子（0-2147483647）
  - 使用相同的种子和参数可以重现相同的图像
...
1. **选择示例提示词**：
   - 从下拉菜单中选择预设的示例提示词（汉服女子、写实人像、奇幻风景）
   - 或选择"自定义"来编写您自己的提示词
...
图像生成后，点击 **"⬇️ 下载图像"** 按钮即可下载 PNG 格式的图像。
...
### Q: 为什么我输入的宽高被自动调整了？
A:
- Z-Image 模型要求图像的宽度和高度必须是16的倍数
- 系统会自动将您输入的值调整为最接近的16的倍数
- 例如：输入1080会自动调整为1072或1088
- 调整后会显示提示信息告知您实际使用的尺寸
